import c3 from 'c3';
import $ from 'jquery';
import { URLS, COLOR_PALETTE } from 'stats/utils';
...
export default class CampaignResultsApplicants extends mix(PlotOptions).with(PlotTypeOptionMixin) {
  ...
  constructor(id, options) {
    ...
    let values = ['accept', 'accept_if', 'volunteer', 'rejected_interview', 'they_refused'];
    this.state = {
  ...
  fetchAndReflow(campaignId) {
    this.getStats(campaignId).then(this.convertData).done(this.reflow);
  }
  ...
  submitButtonHandler = () => {
    ...
    // TODO: cache query
    if (newState !== this.state.type && newState in this.constructor.PLOT_TYPES) {
