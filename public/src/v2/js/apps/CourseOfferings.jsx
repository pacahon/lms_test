...
class CourseOfferings extends React.Component {
  ...
  getYearOptions(branchOption) {
    ...
    const branch = getOptionByValue(
      this.props.branchOptions,
      branchOption.value
    );
    if (branch) {
  ...
  handleMultipleCheckboxChange = onMultipleCheckboxFilterChange.call(this, {
    applyPatches: [this.filteredItemsPatch],
    setStateCallback: this.historyPush
  });
  ...
  render() {
    const { branchOptions, semesterOptions } = this.props;
    const {
      academicYearOptions,
      courseNameQuery,
      academicYear,
          ...
          <div className="col-lg-9">
            <div className="card border-xs-0 mb-4">
              ...
              {!this.state.loading && filteredItems.size <= 0 ? (
                <div className="card__content _big pt-md-0">
          ...
          <Media
            ...
            render={() => (
              <div className="col-lg-3">
                <form className="ui form px-6 mt-6 mt-lg-10 ml-lg-4">
                  <div className="field">
      ...
      <div className={className}>
        ...
        {items
          .filter(function (item) {
            return filteredItems.has(item.id);
          })
          .map(item => (
            <div className="table__row" key={`course-${item.id}`}>
              <div className="table__cell">
                {item.branch.is_club ? <ClubIcon /> : ''}
                <a href={item.url} className="__course">
                  {item.name}
                </a>
              </div>
              ...
              <div className="table__cell _icons">
                ...
                {item.materials.slides ? (
                  <SlidesIcon key={'slides-icon'} />
                ) : (
                  ''
                )}
...
const ClubIcon = () => (
  <Tooltip title="Курс CS клуба">
    <Icon id={'cs-club'} className={`mr-1`} />
  </Tooltip>
);

const VideoIcon = () => (
  <Tooltip title="Видео">
    <Icon id={'video'} className={'mr-1'} />
  </Tooltip>
);

const SlidesIcon = () => (
  <Tooltip title="Слайды">
    <Icon id={'slides'} className={'mr-1'} />
  </Tooltip>
);
